      ...
      <header className="relative z-10 mx-auto flex max-w-6xl items-center justify-between px-6 py-6">
        ...
        {!minimalNav ? (
          <nav className="flex items-center gap-3 text-sm font-semibold">
            <Link
              to="/login"
              className={`rounded-full px-4 py-2 text-violet-800 transition hover:bg-white/60 ${focusRingOnLightClass}`}
            >
              Sign in
            </Link>
            <Link
              to="/register"
              className={`rounded-full bg-linear-to-r from-violet-600 to-fuchsia-500 px-5 py-2.5 text-white shadow-lg shadow-violet-500/25 transition hover:brightness-110 ${focusRingOnLightClass}`}
            >
              Get started
            </Link>
          </nav>
